...
export class AppComponent implements OnInit {
  ...
  onCityInput(input: HTMLInputElement) {
    ...
    fetch(url, {
      ...
      .then((data) => {
        ...
        // for (const [city, state] of this.storeArr) {
        //   if (cityCountMap[city] > 1) {
        //     // If city is duplicated, add "city, state (shorthand)"
        //     this.textOptions.push(`${city}, ${state}`);
        //   } else {
        //     // If city is unique, add "city"
        //     this.textOptions.push(city);
        //   }
        // }
  ...
  onFavoriteClick(city: string, state: string) {
    ...
    let lat, lon;
    const google_url =
      `https://maps.googleapis.com/maps/api/geocode/json?address=${this.city},${this.state}&key=` +
      environment.GOOGLE_API_KEY;
